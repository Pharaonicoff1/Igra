...
export class Player {
   ...
   * @param {boolean} aiming можно ли прямо сейчас прицеливаться — решает ВЫЗЫВАЮЩИЙ
   *   по текущему экрану игры. По умолчанию false: луч — исключение, а не норма,
   *   и любой новый вызов draw() обязан запросить его явно.
   */
  draw(ctx, jumpDistance, scale = 1, aiming = false) {
    const T = theme();
    ...
    //
    // Условие привязано к ЭКРАНУ (через aiming), а не к свойству объекта.
    // Раньше здесь стоял флаг this.decorative — он прикрывал космонавтов меню
    // и магазина, но ничего не знал о настоящем игроке, которого рендер рисовал
    // на всех экранах подряд. Луч из-за этого горел поверх декоративной сцены.
    if (aiming && this.state === STATE_ORBIT) {
      const t = this.tangent();
